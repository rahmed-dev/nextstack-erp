...
- Prefer reusable, cross-module platform systems (DocType engine, naming/numbering, print engine, CSV export, ledger service) over one-off module implementations; modules contribute configuration and domain rules instead of duplicating infrastructure.
...
- **Primary keys:** Each document has a stable string primary key (`name`) used as the canonical identifier in relationships, print formats, exports, and the ops log. For submittable doctypes, this `name` is generated by a central naming/numbering helper (for example `src/core/document/naming.ts`) using per-DocType series patterns defined via configuration (module settings DocTypes such as `AccountingSettings`, `CrmSettings`, `ProjectsSettings`, or a shared `NamingSettings` single DocType), typically scoped per `Entity` for accounting and other entity-bound modules. Pattern changes are forward-only and do not rewrite existing document numbers; external systems (e.g., Upwork) keep their own IDs in separate fields or mapping tables.
...
- **Entity-scoped accounting:** Accounting data is partitioned by an `Entity` DocType (ERPNext-like "Company") per workspace. Every ledger-affecting document (accounts, parties, invoices, payments, expenses, GL entries, etc.) carries an `entity` link; default accounts and behavioral configuration (numbering series, ageing thresholds) are driven by per-Entity `Entity` + `AccountingSettings` records. Cross-Entity combinations within a single document are invalid and blocked during validation, and an Entity’s `base_currency` is immutable once there is accounting activity for that Entity.
- **Ledger engine:** General Ledger lines are modeled via an append-only `GLEntry` DocType and posted through a dedicated `ledgerService` in `src/core/ledger`. Domain services for invoices, payments, expenses, and journal entries submit balanced batches of proposed lines; the ledger engine enforces entity consistency, debits-equal-credits (within tolerance), and account validity before persisting.
- **Print & export mechanisms:** A DocType-agnostic print engine in `src/core/document/print.ts` renders documents using reusable `PrintFormat` definitions, and a generic CSV export utility in `src/core/document/export.ts` plus view-specific schemas under `src/modules/*/domain/export/` provides stable, documented exports (GL, invoices, payments, expenses). These mechanisms are read-only and never modify document lifecycle or accounting state.
